...
class ItemControllerTest implements WithUserHeaderID {
    ...
    @Test
    @SneakyThrows
    void createInvalidItemFails() {
        Long userId = 0L;
        ItemDto itemDto = MapperItem.toItemDto(item).toBuilder()
                .name(" ")
                .description(" ")
                .available(null)
                .build();
        ...
        verify(itemService, never()).add(userId, itemDto);
    }
    ...
    @Test
    @SneakyThrows
    void searchWithInvalidPagingParams() {
        Integer from = -1;
        Integer size = 10;
        String text = "item";
