...
class BookingControllerTest implements WithUserHeaderID {
    ...
    @Test
    @SneakyThrows
    void shouldFailAddRequestOnInvalidBooking() {
        BookingDto invalidBookingDto = bookingDto.toBuilder()
                .itemId(null)
                .start(null)
                .end(null)
                .build();

        when(bookingService.add(user.getId(), invalidBookingDto))
                .thenReturn(bookingResponseDto);
        ...
        verify(bookingService, never()).add(user.getId(), invalidBookingDto);
    }
    ...
    @Test
    @SneakyThrows
    void shouldFailToFindAllBookingsByUserId() {
        Integer from = -1;
        Integer size = 10;
        String state = "ALL";
        ...
        verify(bookingService, never()).findAllByBookerId(user.getId(), state, from, size);
    }
    ...
    @Test
    @SneakyThrows
    void shouldFailToFindAllByOwnerId() {
        Integer from = -1;
        Integer size = 10;
        String state = "ALL";
        ...
        verify(bookingService, never()).findAllByOwnerId(user.getId(), state, from, size);
    }
}
